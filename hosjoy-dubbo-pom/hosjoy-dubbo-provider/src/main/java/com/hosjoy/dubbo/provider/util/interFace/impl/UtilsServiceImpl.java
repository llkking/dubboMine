...
public class UtilsServiceImpl implements IUtilsService{
    ...
    @Override
    public boolean sendEmailService(List<String> receiverEmails,String context,String topic) throws Exception {
     ...
     // 根据邮件配置创建会话，注意session别导错包
        ...
        //创建邮件
        MimeMessage message = createEmail(session,receiverEmails,context,topic);
    ...
    public MimeMessage createEmail(Session session,List<String> receiverEmails,String context,String topic) throws Exception {
    	// 1. 创建一封邮件根据session
        ...
        if(receiverEmails.size()>1){
        	...
        	for (String receiverEmail : receiverEmails) {
        		msg.addRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiverEmail, "", "UTF-8"));
			}
        }
